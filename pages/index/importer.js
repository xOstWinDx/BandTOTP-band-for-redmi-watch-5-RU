...
function compactMeta(item) {
  ...
  const name = item.name || item.issuer || item.service || item.serviceName || DEFAULT_NAME;
  return {
    name: String(name || DEFAULT_NAME),
    usr: String(item.usr || item.user || item.account || item.accountName || item.email || "")
  };
}

function normalizeMeta(rawMeta) {
  const items = Array.isArray(rawMeta) ? rawMeta : [];
  const result = [];

  for (let i = 0; i < items.length; i++) {
    const meta = compactMeta(items[i]);
    if (meta) {
      result.push(meta);
    }
  }
...
function readImportPayload(rawData) {
  const parsed = safeJsonParse(rawData);

  if (parsed && typeof parsed === "object" && parsed.vault) {
    return {
      vault: parsed.vault,
  ...
  return {
    accounts: normalizeAccounts(rawData)
  };
}

export { normalizeAccounts, readImportPayload };
